...
def conv2d(x, output_dim, k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02, initializer=None, padding='SAME', name='conv2d',
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        conv = tf.compat.v1.nn.conv2d(x, w, strides=[1, d_h, d_w, 1], padding=padding)
...
def batch_norm(x, name, _ops, is_train=True, is_print=True, logger=None):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        if is_train is True:
            ...
            #     mean, variance = tf.compat.v1.nn.moments(x, [0, 1, 2], name='moments')
            # elif len(x.get_shape().as_list()) == 2:
            #     mean, variance = tf.compat.v1.nn.moments(x, [0], name='moments')  # 0
            index = [i for i in range(len(x.get_shape().as_list()[:-1]))]
            # print('index:{}'.format(index))
            mean, variance = tf.compat.v1.nn.moments(x, index, name='moments')
